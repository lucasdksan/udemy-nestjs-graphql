...
import { UnpublishPost } from "../unpublish-post.usecase";

jest.setTimeout(30000); 

describe("UnpublishPostUsecase Integration tests", ()=> {
    let module: TestingModule;
    let postsRepository: PostsPrismaRepository;
    let authorsRepository: AuthorsPrismaRepository;
    let usecase: UnpublishPost.Usecase;
    ...
    beforeAll(async ()=> {
        ...
        usecase = new UnpublishPost.Usecase(postsRepository);
    });
    ...
    test("Should it changes the published state to false", async ()=> {
